...
const BodyQuestion = ({
  ...
  let [bodyQuestionArticle, setBodyQuestionArticle] = useState(null);
  ...
  const handleArticleMessagePost = (e) => {
    if (say.length === 0) {
      window.alert("留言不能空白");
      navigate("/articles/bodyquestion");
    } else {
      articleService
  ...
  useEffect(() => {
    articleService
      ...
      .then((data) => {
        ...
        setBodyQuestionArticle(data.data);
      })
      .catch((err) => {
        console.log(err);
      });
  }, [bodyQuestionArticle, setLoading]);
  ...
  return (
    <div className="allArticleBackground" id="allArticleBackground">
      ...
      {currentUser &&
        bodyQuestionArticle &&
        loading === false &&
        bodyQuestionArticle !== 0 && (
          <div className="allArticleContainer">
            {bodyQuestionArticle.map((article) => (
